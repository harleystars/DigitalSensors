...
#define SERIAL_R 39000 // сопротивление  резистора, 39 кОм
#define THERMISTOR_R 43000 // номинальное сопротивления термистора, 43 кОм
#define NOMINAL_T 25 // номинальная температура (при которой TR = 43 кОм)

void setup() {
  Serial.begin(9600);
  pinMode(A0,INPUT);  //режим пина А0 с подтяжкой по питанию, чтение давления
  pinMode(A1,INPUT);  //пин А1 переведен в чтение с него напряжения, температура
 }
...
//Чтение, обработка и пересчет показаний с термистора NTC, пин А1 (взято из инета)
  ...
  float Pressure=map(analogRead(A0),96,920,0,150);      //давление в psi 
  ...
  {Serial.print(kpa,2); //вывод значения давления в kPa в serial порт
  Serial.println("c");}
